class Division < ActiveRecord::Base
  ...
  def update_store_counts
    if company_id_changed?
      ...
      Company.update_counters company_id_was, :stores_count => (-1)*self[:stores_count], :divisions_count => -1
      Company.update_counters self[:company_id], :stores_count => self[:stores_count], :divisions_count => 1   
    end
  end
end